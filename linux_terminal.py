...
__all__ = [
     'Switcher', 'Brightness',  'ValueBrightnessError', 'Creator', 'PhotoCapture', 'AppSystem', 'ScreenCapture'
]
    ...
    class PhotoCapture(object):
        ...
        def capture(self, cam_index: int, extension, filename):
            ...
            if subprocess.getstatusoutput(
                cmd=f'/opt/local/bin/ffmpeg  -f avfoundation -video_size {1280}x{720} -framerate 30 -i "{cam_index}" -vframes 1 {filename}.{extension}')[0] == 1:
